...
exports.doCharge = function (req, res, next) {
  ...
  var chargeToken =
    "Basic " +
    new Buffer("SB-Mid-server-hIhpmxYRUKEKSZqtquUvcTvm").toString("base64"); // Dummy Charge Token
...
exports.doChargeProd = function (req, res, next) {
  ...
  var chargeToken =
    "Basic " +
    new Buffer("SB-Mid-server-hIhpmxYRUKEKSZqtquUvcTvm").toString("base64");
...
exports.checkauth = function (req, res) {
  ...
  if (checkTokenValidity(token)) {
    res
      .status(200)
      .json({ status_code: 200, status_message: "Token is valid" });
  } else {
    res
      .status(403)
      .json({ status_code: 403, status_message: "Token is invalid" });
  }
};
    ...
    if (isTokenValid) {
      ...
      if (cardFound) {
        ...
        if (success) {
          res
            .status(200)
            .json({ status_code: 200, status_message: "Card is delted" });
        } else {
          res.status(500).json({
            status_code: 500,
        ...
        res
          .status(404)
          .json({ status_code: 404, status_message: "Card is not found" });
      }
    } else {
      res
        .status(403)
        .json({ status_code: 403, status_message: "Invalid X-Auth token" });
    }
  } else {
    if (!token) {
      res
      ...
      res
        .status(404)
        .json({ status_code: 404, status_message: "Card not found" });
    }
  }
};
  ...
  if (
    ...
    if (isTokenValid) {
      ...
      if (success) {
        if (!cardAlreadySaved) {
          res
            .status(201)
            .json({ status_code: 201, status_message: "Card is saved" });
        } else {
          res.status(208).json({
            status_code: 208,
            status_message: "Card is already saved",
          });
        }
      } else {
        res
    ...
    if (!token) {
      ...
      res.status(400).json({
        status_code: 400,
        status_message:
          "status_code dari papi harus 200, harus ada saved_token_id, harus ada masked_card",
      });
    }
  }
};
